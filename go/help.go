...
func printCreateHelp() {
	...
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]'")
	fmt.Println()
	fmt.Println("  # Print the request of the job using a device on the default namespace.")
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]' --dry-run")
	fmt.Println()
	fmt.Println("  # Run a job using a device on the default namespace for debugging purposes.")
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]' --debug")
	fmt.Println()
	fmt.Println("  # Run a job that should be deleted after 10 minutes.")
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]' --ttl 600")
	fmt.Println()
	fmt.Println("  # Run a job that is allowed to backoff 3 times.")
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]' --backoff-limit 3")
	fmt.Println()
	fmt.Println("  # Run a job that must complete 2 times.")
	fmt.Println("  kubectl job-manager create -j my-job -n default -d xilinx.com/fpga -i busybox -c '[\"fmt.Println(\", \"Hello!\"]' --completions 2")
	fmt.Println()
	fmt.Println("Options:")
